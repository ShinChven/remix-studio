...
| `server/media-share/dlna/` | `DlnaService` (routes under `/dlna/:id/`, SOAP, eventing, media), `ssdp.ts` (discovery), `content-directory.ts` (object tree + DIDL-Lite), `xml.ts` (descriptions, SCPDs). |
...
- DLNA answers only `isLocalPeer` addresses and only when `DLNA_ENABLED` is set.

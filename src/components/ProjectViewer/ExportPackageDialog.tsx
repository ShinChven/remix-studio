...
export function ExportPackageDialog({
  ...
  return (
    <div className="fixed inset-0 z-[320] flex items-center justify-center p-3 sm:p-4">
      ...
      <div
        className="relative w-full max-w-lg max-h-[calc(100dvh-1.5rem)] sm:max-h-[min(720px,calc(100dvh-2rem))] overflow-hidden rounded-[24px] sm:rounded-[32px] border border-neutral-800 bg-neutral-900 shadow-[0_50px_100px_rgba(0,0,0,0.8)] animate-in zoom-in-95 duration-300"
      ...
      >
        <div className="flex items-center justify-between gap-3 border-b border-neutral-800 bg-neutral-950/30 p-4 sm:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-black tracking-tight text-white">{t('projectViewer.exportDialog.title')}</h3>
        ...
        <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 border-t border-neutral-800 bg-neutral-950/40 p-4 sm:p-6">
          ...
          <button
            ...
            className="flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl sm:rounded-2xl bg-blue-600 px-6 py-3 text-[10px] font-black uppercase tracking-widest text-white shadow-lg shadow-blue-500/20 transition-all hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
          >

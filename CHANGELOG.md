...
- **Current Account Tool for the Assistant and MCP**: Every tool in the registry acts on the account behind the token, but nothing reported which account that is — an agent connected over MCP could read the user's libraries and never be able to answer "who am I signed in as?", and had no way to check whether a token belonged to the expected account before writing with it. `get_current_account` returns the identity the session resolves to: user id, email, role, account status, storage limit, whether a password and two-factor are set, the created/updated/last-login timestamps, and the link to the account page. It takes no arguments, reads only the caller's own record — it cannot look up another user — and exposes no password hash or token material. Storage consumption still belongs to `get_storage_usage`; this tool reports the limit only.

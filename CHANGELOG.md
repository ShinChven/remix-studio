...
- **MiniMax Hailuo H3 Video (RunningHub)**: Added `minimax/hailuo-h3` as a RunningHub video model. It runs on the standard-model API the other RunningHub models use — submit, then poll `/query` from the detached poller — but its request body is much narrower than Seedance's: prompt, an optional first and last frame, resolution (`2K` or `768P`), and a duration of 5 to 15 seconds. Ratio, sound, real-person mode, and the conversion slots Seedance takes are not part of it, so the generator now builds a dedicated payload for Hailuo instead of stripping fields off the Seedance one. A prompt is required on every request, the aspect ratio is fixed to `adaptive` because the output follows the first frame, and a resolution or duration carried over from another video model is mapped onto the nearest value Hailuo accepts rather than being sent through and rejected. The generator also trims a configured API URL back to the API root, so a URL pasted from any model's endpoint page still addresses the model the project selected.

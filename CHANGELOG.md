...
- **GPT Image 2.5 on RunningHub**: RunningHub now serves GPT Image 2.5, and both tiers are selectable on any RunningHub provider: **GPT Image 2.5 Sunburst** (`rhart-image-g-2.5/sunburst`), the base model tuned for quality, and **GPT Image 2.5 Flare** (`rhart-image-g-2.5/flare`), the small, speed-tuned one — the same split OpenAI ships directly, now available through RunningHub's wallet instead of an OpenAI key. Both offer fifteen aspect ratios (`1:1` through `3:1` and `1:3`, including the `2:1`, `1:2`, `21:9` and `9:21` wide formats), the `1K` / `2K` / `4K` resolution tiers, and text-to-image as well as reference-image editing with up to ten references. Prompts are capped at 20,000 characters, which the shared draft validation applies before a job is submitted. Neither entry needed generator code. RunningHub addresses the 2.5 tiers as a two-segment path — the model plus its tier — rather than the flat slug its older `rhart-` models use, and the generator already builds a submit URL by appending the endpoint to whatever the model ID is, so `rhart-image-g-2.5/sunburst` resolves to `.../rhart-image-g-2.5/sunburst/text-to-image` unchanged. Their request body is the default RunningHub image shape (a prompt, a resolution tier, an optional aspect ratio, and the uploaded reference URLs), and their reference endpoint is the default `/image-to-image`, so neither needs the per-model branch that Qwen, Wan, Seedream and the Grok and nano banana Pro tiers each carry. One option is deliberately missing: the documented ratio enum has no `auto` value on either endpoint, unlike nano banana 2 and Grok Imagine Quality, so the pickers do not offer it — the generator treats `auto` as "omit the field", which on these models would quietly drop the choice rather than report it. The existing RunningHub entries are untouched: `GPT Image 2` and `GPT Image 2 Official` keep their model IDs, their tiered quality picker and their behaviour, and projects pinned to them are unaffected.

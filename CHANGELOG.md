...
- **Moving Album Items Reported a Failure After It Had Worked**: The move endpoint did its housekeeping — sweeping up the source copies of the files it had just moved, then publishing the live events — inside the same `try` as the move itself, so anything that went wrong in that tail turned a completed move into `Failed to move album items`. The items really had moved, and the confirmation page stayed put reporting an error over work that had already succeeded. The move through the repository is now the point of no return: the sweep and the event publishing sit in their own `try`, and a failure in either is logged and swallowed rather than answered with a 500, leaving at worst a few source files for the orphan tool. A second, harder failure had the same message: an album row can reference a file that is no longer in the bucket — a reference image replaced in the workflow, a step deleted, a result already cleaned up — and `CopyObject` throws on a missing source, so one stale reference aborted the whole move before anything had moved. Referenced keys are now checked before the copy and the missing ones are left where they are, which keeps the move going and reports how many were skipped. A destination project created for a move that then fails is deleted again if nothing landed in it, instead of being left behind empty, and the 500 now carries the underlying reason instead of a bare sentence. The storage round trips — existence checks, copies and deletes, three per item — also ran strictly one at a time, so a few hundred images meant thousands of sequential requests and a wait long enough for a proxy to give up on the request while the server carried on; they now run sixteen at a time.

...
function createMcpServerInstance(repository: IRepository, userRepository: UserRepository, prisma: PrismaClient, providerRepository: ProviderRepository, userId: string) {
  ...
  server.registerTool(
    ...
    async () => {
      ...
      // Build a clean catalog: group built-in models by provider type
...
Use this tool to find an item by name/title before composing a workflow. Combine with list_all_libraries to first discover the libraryId.`,
    ...
    async ({ library_id, query, page, limit }) => {
      // Verify library ownership and get its type
      const library = await repository.getLibrary(userId, library_id);
      if (!library) {
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ error: 'Library not found' }) }],
          isError: true,
        };
      }
  ...
  // ─── Tool: create_project_with_workflow ───
  server.registerTool(
...
4. Call create_project_with_workflow with assembled data.`,
    ...
    async ({ name, type, providerId, modelConfigId, aspectRatio, quality, shuffle, prefix, systemPrompt, temperature, maxTokens, duration, resolution, sound, workflowItems }) => {
      // Map user-facing itemType → internal WorkflowItem type + value
      ...
      const project = {
        id: projectId,
        name,
        type: type as 'image' | 'text' | 'video' | 'audio',
...
export function createMcpRouter(prisma: PrismaClient, repository: IRepository, userRepository: UserRepository, providerRepository: ProviderRepository) {

...
- Album items carry tags. \`get_album_items\` returns each item's tags plus a \`tagCounts\` roll-up of every tag in the album, and takes \`tags\` / \`tag_match\` to filter. \`tag_album_items\` writes them: pass \`item_ids\` for a specific set, or \`all_items\` with optional \`filter_tags\` / \`aspect_ratios\` to retag a whole slice in one call. Use exactly one of \`add\`, \`remove\` or \`replace\` per call — \`replace\` discards the item's other tags, so reach for \`add\` unless the user asked for the tag list to become exactly something.

...
const VALID_TYPES: ProviderType[] = ['GoogleAI', 'VertexAI', 'RunningHub', 'KlingAI', 'OpenAI', 'Grok', 'Claude', 'BytePlus', 'Replicate', 'BlackForestLabs', 'Alibabacloud', 'Kimi', 'MiniMax'];
...
export function createProviderRouter(repo: ProviderRepository) {
  ...
  router.get('/api/providers/:id/models', authMiddleware, async (c) => {
    try {
      ...
      if (providerType === 'RunningHub' || providerType === 'KlingAI' || providerType === 'BytePlus' || providerType === 'Replicate' || providerType === 'BlackForestLabs') {

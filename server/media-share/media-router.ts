...
/** Validates a scope from a request body: null for every project, else owned project ids. */
async function parseScope(prisma: PrismaClient, userId: string, value: unknown): Promise<string[] | null | 'invalid'> {
  ...
  const owned = await prisma.project.count({ where: { userId, id: { in: ids } } });
  return owned === ids.length ? ids : 'invalid';
}

function serializeDevice(device: MediaDevice) {
  return {
    id: device.id,
    name: device.name,
    kind: device.kind,
    tokenPrefix: device.tokenPrefix,
    projectIds: scopeFromJson(device.projectIds),
...
export function createMediaRouter(
  ...
  router.get('/api/media-devices', authMiddleware, async (c) => {
    ...
    const devices = await prisma.mediaDevice.findMany({ where: { userId: user.userId }, orderBy: [{ createdAt: 'desc' }, { id: 'desc' }] });
    return c.json({
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      try {
        ...
        return c.json({
          ...
          linkPath: `/tv/link?code=${userCode}`,
        });
      } catch (error) {
  ...
  async function tvFolder(folder: MediaFolder) {
    return {
      ...
      coverUrl: folder.cover ? await sign(folder.cover.thumbnailKey || folder.cover.optimizedKey) : null,
    };
  }
